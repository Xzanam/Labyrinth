...
void Maze::AddBox(glm::vec3 min , glm::vec3 max) {
    ...
    for (int f = 0 ; f < 6 ; ++f) {
        ...
        //Add two triangles
        mesh.PushIndex(base + f* 4  + 0);
        mesh.PushIndex(base + f* 4 + 1);
        mesh.PushIndex(base + f* 4 + 2);


        mesh.PushIndex(base + f* 4 + 0);
        mesh.PushIndex(base + f* 4 + 2);
        mesh.PushIndex(base + f* 4 + 3);
    }
...
void Maze::GenerateMesh(float cellSize, float wallThickness, float wallHeight) {
    mesh.Clear();
    m_wallAABBs.clear();
